...
- `.token-exchange-signing-key.pem` — RSA signing key for Token Exchange (chmod 600)
...
- Signs fused delegation tokens with RS256 (persistent key from `SIGNING_KEY_PATH`, generated by bootstrap)
...
| `SIGNING_KEY_PATH` | `/run/secrets/signing-key.pem` | token-exchange | Path to PEM private key for RS256 signing (generated by bootstrap, mounted by compose) |
